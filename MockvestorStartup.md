...
*Note: This automated setup supports both Windows and macOS environments and requires Python 3.13.5 to be installed on your machine.*

## 1. Project Initialization
1. Download the repository zip file from the `main` branch.
...
2. **Find your IP Address:** Your computer will act as the local backend server. 
   - **Windows (Powershell/CommandPrompt):** Run `ipconfig` and look for "IPv4 Address":
   - **macOS (Terminal):** Run `ipconfig getifaddr en0` (for Wi-Fi) or check your IP under System Settings > Network.
...
- **For macOS:**
   1. Open Terminal and navigate to the project's root directory.
   2. Grant execution permissions to the script (you only need to do this once) by running:
   ```Terminal
   chmod +x start-mockvestor.sh
   ```
   3. Execute the script to initiate the automated launch sequence:
   ```Terminal
   ./start-mockvestor.sh
   ```
